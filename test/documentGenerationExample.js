...
const sendEnvelope = async (args) => {
  ...
  const template = await templatesApi.createTemplate(accountId, {
  ...
  const documentId = "1";
  await templatesApi.updateDocument(accountId, templateId, documentId, {
    envelopeDefinition: documentData,
  });
  ...
  const recipientId = "1";
  await templatesApi.createTabs(accountId, templateId, recipientId, {
    templateTabs: tabs,
  });
  ...
  const docGenFormFieldsResponse =
    await envelopesApi.getEnvelopeDocGenFormFields(accountId, envelopeId);
  const documentIdGuid =
    docGenFormFieldsResponse.docGenFormFields[0].documentId;
  //ds-snippet-end:eSign42Step6

  //ds-snippet-start:eSign42Step7
  const formFieldsData = formFields(documentIdGuid, envelopeArgs);
  await envelopesApi.updateEnvelopeDocGenFormFields(accountId, envelopeId, {
    docGenFormFieldRequest: formFieldsData,
  });
  ...
  return await envelopesApi.update(accountId, envelopeId, {
    envelope: sendEnvelopeReq,
  });
